...
{#if $modalStore[0]}
    <div class="card p-3 sm:p-5 space-y-6 flex flex-col max-w-[600px] w-full relative">
        <div>
        ...
        >
            <div>
                ...
                {#if $errors.name || nameNotAvailable}
                    <div class="mt-2">
                        ...
                        {/if}
                        {#if nameNotAvailable}
                            <p class="text-[14px] text-error-400 whitespace-normal">Name is already being used</p>
                        {/if}
                    </div>
                {/if}
            </div>
            ...
            <div>
                <label for="readme" class="label">
                    ...
                    />
                    <Button type="button" class="btn px-1 chip variant-ghost-surface" on:click={() => preview = !preview}>{preview ? 'Hide Preview' : 'Show Preview'}</Button>
