<script lang="ts">
    ...
    async function submit() {
        ...
        const response = await stexs.auth.mfa.disable('email', $form.code);

        if (response.ok) {
            $flash = {
                message: 'Email authentication method successfully disabled.',
...
{#if $modalStore[0]}
    <div class="card p-5 space-y-6 flex flex-col relative max-w-[380px] w-full">
        ...
        <p class="text-center">Enter the code sent to your email to disable email authentication.</p>
        ...
        >
            ...
            <div class="flex justify-between w-full">
                ...
                <Button type="submit" class="variant-ghost-surface px-2 py-1 text-red-600">Disable</Button>
            </div>
        </form>
    </div>
{/if}
