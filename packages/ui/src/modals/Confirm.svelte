    ...
    <div class="card p-5 space-y-6 flex items-center flex-col">
        ...
        <div class="flex justify-between w-full">
            ...
            <Button on:click={async () => { 
                submitted = true;
                await Promise.resolve($modalStore[0].meta.fn($modalStore[0].meta.fnParams));
                if (close) modalStore.close();
            }} class={confirmBtnClass} loaderMeter={confirmBtnLoaderMeter} loaderTrack={confirmBtnLoaderTrack} {submitted}>{confirmBtnText}</Button>
        </div>
    </div>
{/if}
