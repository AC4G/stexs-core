<script lang="ts">
    ...
    import { openChangePasswordModal, openChangeEmailModal, openEnableTOTPModal, openRemoveTOTPModal, openDisableEmailModal, openEnableEmailModal, openSignOutFromAllSessionsModal } from "$lib/utils/modals/userModals";
...
<div class="px-[4%] md:px-[8%] grid place-items-center">
    <div class="w-full my-[40px] lg:max-w-[1200px] space-y-6">
        ...
        {:else}
            <div class="space-y-2">
                <p class="text-surface-300 w-fit">Active Sessions ({$sessionsAmountQuery.data.amount})</p>
                <Button class="variant-filled-primary px-2 py-1" on:click={() => openSignOutFromAllSessionsModal(modalStore, openMFAModal)}>Sign Out from All Sessions</Button>
            </div>
        {/if}
    </div>
</div>
