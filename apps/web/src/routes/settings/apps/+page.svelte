<script lang="ts">
    ...
    import { getModalStore, ListBoxItem, Paginator, type PaginationSettings } from "@skeletonlabs/skeleton";
    ...
    $: connectionsQuery = createQuery({
        ...
        queryFn: async () => await fetchConnections($userStore?.id!, search, filter, typeFilter, paginationSettings.page, paginationSettings.limit),
        enabled: !!$userStore?.id
    })
</script>
...
<div class="px-[4%] md:px-[8%] grid place-items-center mb-[18px]">
                ...
                <div class="w-full xs:w-fit flex flex-col xs:flex-row items-center space-y-2 xs:space-y-0 xs:space-x-2">
                    <div class="w-full xs:w-fit">
                        ...
                        <Dropdown class="rounded-md bg-surface-800 p-2 space-y-2 border border-surface-500">
                            <ListBoxItem bind:group={filter} name="filter" value={'A-Z'} active="variant-glass-primary text-primary-500" hover="hover:variant-filled-surface" class="rounded-md px-4 py-2">A-Z</ListBoxItem>
                            <ListBoxItem bind:group={filter} name="filter" value={'Z-A'} active="variant-glass-primary text-primary-500" hover="hover:variant-filled-surface" class="rounded-md px-4 py-2">Z-A</ListBoxItem>
                            <ListBoxItem bind:group={filter} name="filter" value={'Latest'} active="variant-glass-primary text-primary-500" hover="hover:variant-filled-surface" class="rounded-md px-4 py-2">Latest</ListBoxItem>
                    ...
                    <div class="w-full xs:w-fit">
                        <Button class="bg-surface-500 border border-gray-600 w-full xs:w-fit py-[8px]">{typeFilter}<Icon
                            ...
                            /></Button>
                        <Dropdown class="rounded-md bg-surface-800 p-2 space-y-2 border border-surface-500">
                            <ListBoxItem bind:group={typeFilter} name="filter" value={'All'} active="variant-glass-primary text-primary-500" hover="hover:variant-filled-surface" class="rounded-md px-4 py-2">All</ListBoxItem>
                            <ListBoxItem bind:group={typeFilter} name="filter" value={'Organizations'} active="variant-glass-primary text-primary-500" hover="hover:variant-filled-surface" class="rounded-md px-4 py-2">Organizations</ListBoxItem>
                            <ListBoxItem bind:group={typeFilter} name="filter" value={'Projects'} active="variant-glass-primary text-primary-500" hover="hover:variant-filled-surface" class="rounded-md px-4 py-2">Projects</ListBoxItem>
                        </Dropdown>
                    </div>
                </div>
            </div>
    ...
    <div class="flex flex-col w-full space-y-4">
        ...
        {:else}
            ...
            {:else}
                <div class="grid place-items-center rounded-md col-span-full">
                    <p class="text-[18px] p-6 text-center">You have no apps connected to your account</p>
                </div>
            {/if}
        {/if}
    </div>
