  ...
  let authorizeSetupQuery = createQuery({
    ...
    queryFn: async () => {
      ...
      filteredNodes = scopesTreeViewNodes
        .map((node) => {
          const filteredChildren = node.children?.filter((child) => scopes.includes(child.id));
        ...
        .filter((node) => node.children && node.children.length > 0 || !node.children);
  ...
  <div class="card p-5 variant-ghost-surface space-y-6 w-full max-w-[400px]">
    ...
    {#if $authorizeSetupQuery.isLoading || !$authorizeSetupQuery.data}
      <div class="flex flex-row items-center justify-evenly space-x-2">
        <div class="w-[60px] h-[60px] xs:w-[80px] xs:h-[80px] rounded-full placeholder animate-pulse"/>
    ...
    {:else}
      <div class="flex flex-row items-center justify-evenly space-x-2">
        <div class="w-[60px] xs:w-[80px] overflow-hidden !rounded-full bg-surface-800 border-2 border-surface-600">
          <OrganizationLogo {stexs} organizationId={$authorizeSetupQuery.data.organization_id} alt={$authorizeSetupQuery.data.organization_name} iconClass="text-[46px]" />
        </div>
        <Icon  icon="tabler:dots" width="34" />
        <Avatar {stexs} userId={$userStore?.id || ""} username={$userStore?.username} class="w-[60px] xs:w-[80px] !bg-surface-800 border-2 border-surface-600" draggable="false" />
      </div>
      <p class="text-center break-all text-[22px] font-medium">{$authorizeSetupQuery.data.name}</p>
      <div class="space-y-2">
        <p class="text-center break-all text-[16px]">by
          <a href="/organizations/{$authorizeSetupQuery.data.organization_name}" class="text-secondary-500 hover:text-secondary-400 transition">{$authorizeSetupQuery.data.organization_name}</a>
          {$authorizeSetupQuery.data.project_name ? ' for ' : ''}
          {#if $authorizeSetupQuery.data.project_name}
            <a href="/organizations/{$authorizeSetupQuery.data.organization_name}/projects/{$authorizeSetupQuery.data.project_name}" class="text-secondary-500 hover:text-secondary-400 transition">{$authorizeSetupQuery.data.project_name}</a>
          {/if}
          <br>wants to gain access to your account
        </p>
        <div class="flex flex-row space-x-2 justify-center">
          <p class="text-[14px] break-all text-surface-300">Signed in as {$userStore?.username}</p>
          <Button class="p-0 text-[14px] text-secondary-500 hover:text-secondary-400" on:click={async () => {
      ...
      <div>
        ...
        <p class="text-[14px] text-surface-300 flex flex-row">
          ...
          App active since: {new Date($authorizeSetupQuery.data.created_at).toLocaleString()}
        </p>
      </div>
    {/if}
  </div>
