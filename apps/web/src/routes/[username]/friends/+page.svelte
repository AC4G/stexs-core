		...
		{#if $userStore?.id === $profileStore?.userId}
			...
			>
				...
				<div
					...
					data-popup="addFriendProfilePopup"
				>
...
>
	...
	{:else if $friendsQuery.data?.length > 0}
		{#each $friendsQuery.data as friend (friend.id)}
			<div
				class="flex flex-row rounded-md transition items-center p-2 w-full bg-surface-700 border border-surface-600"
			>
				<a
					href="/{friend.profiles.username}"
					class="flex h-full w-full items-center justify-left group"
				>
					<Avatar
						class="w-[40px] h-[40px] !bg-surface-800 border-2 border-surface-600 group-hover:border-primary-500 transition"
						userId={friend.profiles.user_id}
						username={friend.profiles.username}
						{stexs}
					/>
					<p
						class="text-[14px] w-[70%] text-left pl-2 break-all group-hover:text-secondary-400 transition"
					>
				...
				{#if $userStore?.id === $profileStore?.userId}
					...
					<Dropdown
						bind:open={openDropDown[friend.id]}
						placement="left"
