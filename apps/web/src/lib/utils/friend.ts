...
export async function removeFriend(userId: string, friendId: string, flash: Writable<ToastSettings>) {
...
export async function revokeFriendRequest(requesterId: string, addresseeId: string, flash: Writable<ToastSettings>) {
...
export async function sendFriendRequest(username: string, requester_id: string, addressee_id: string, flash: Writable<ToastSettings>) {
  const { error } = await stexs.from('friend_requests')
      .insert([
